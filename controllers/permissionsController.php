...
class permissionsController extends controller {
    ...
    public function index() {
        ...
        if ($users->hasPermission('permissions')) {
            ...
            $data['pagina_params'] = 1;
            if (isset($_GET['p_params']) && !empty($_GET['p_params'])) {
                $data['pagina_params'] = intval($_GET['p_params']);
                if ($data['pagina_params'] == '0') {
                    $data['pagina_params'] = 1;
                }
            }

            $offiset_params = (10 * ($data['pagina_params']-1));

            $offiset_groups = 0; 

            $data['pagina_groups'] = 1;
            if (isset($_GET['p_groups']) && !empty($_GET['p_groups'])) {
                $data['pagina_groups'] = intval($_GET['p_groups']);
                if ($data['pagina_groups'] == '0') {
                    $data['pagina_groups'] = 1;
                }
            }

            $offiset_groups = (10 * ($data['pagina_groups']-1));

            $permissions = new Permissions();
            $data["permission_params_list"] = $permissions->getListParams($offiset_params);
            $data["permission_groups_list"] = $permissions->getListGroups($offiset_groups);

            $data["permission_params_count"] = $permissions->getCountParams($users->getSubscriber());
            $data["paginas_params_count"] = ceil( $data["permission_params_count"] / 10 );

            $data["permission_groups_count"] = $permissions->getCountgroups($users->getSubscriber());
            $data["paginas_groups_count"] = ceil( $data["permission_groups_count"] / 10 );
    ...
    public function add_group() {
        ...
        if ($users->hasPermission('permissions') && $users->hasPermission('permission_groups_add')) {
            $permissions = new Permissions();
            $data["permission_params_list"] = $permissions->getListParams(-1);
